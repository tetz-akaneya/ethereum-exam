...
const bigintToBuffer = (n: bigint, byteSize: number): Buffer => {
  if (n < 0n) {
    throw new Error('Only non-negative integers are supported');
  }
...
export const selfmadeCKDpriv = (privKey: Buffer, chainCode: Buffer, index: number) => {
  ...
  const childKeyBn = toFinateValue(bufferToBigInt(IL) + bufferToBigInt(privKey), CURVE_ORDER);
...
const toFinateValue = (n: bigint, p: bigint) => {
  return ((n % p) + p) % p;
}
 ...
 * 拡張ユークリッドの互除法を使う
 */
export const inverseOfFiniteP = (a: bigint, p: bigint): bigint => {
  a = toFinateValue(a, p);
  let [t, newT] = [0n, 1n];
  let [r, newR] = [p, a];
...
export function pointAdd(P: Point, Q: Point): Point {
  ...
  if (x1 === x2 && y1 === y2) {
    // 倍加（doubling）
    (lambda = 3n * x1 * x1 * inverseOfFiniteP(2n * y1, modP)), modP;
  } else {
    // 一般加算
    (lambda = (y2 - y1) * inverseOfFiniteP(x2 - x1, modP)), modP;
  }

  const x3 = toFinateValue(lambda * lambda - x1 - x2, modP);
  const y3 = toFinateValue(lambda * (x1 - x3) - y1, modP);
